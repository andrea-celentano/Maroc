...
bool TAnalysis::PlotChannel(int channel,bool save, int xmin,int xmax,int ymax, int multiplicity){	
	...
	if (!this->LoadTree(fnomeroot)) {
		...
		if (!this->IsGoodSystemChannel(channel)){
			...
			TH1F * hsinglediscri = new TH1F("hsinglediscri","digital output single",4096,-0.5,4095.5);	
			...
			ftree->Draw(Form("ADC[%d]>>hsinglediscri",channel),Form("(hit[%d]==1)&&(EvtMultiplicity==1)",channel),"same");
			hsinglediscri->SetFillColor(kBlue);
			...
			fparam_singlehit[idx].entries	= hsinglediscri->GetEntries();
			fparam_singlehit[idx].mean		= hsinglediscri->GetMean();
			fparam_singlehit[idx].rms		= hsinglediscri->GetRMS();
			...
			Int_t lbminsingleADChit = 0;
			Int_t lbmaxsingleADChit = 0;
			Int_t ksinglehit = 0;
			for(int j=0; j<hsinglediscri->GetNbinsX()+1; j++){
				if(hsinglediscri->GetBinContent(j)!=0){
					if(ksinglehit==0){
						lbminsingleADChit=j;
					}
					ksinglehit++;
			...
			fparam_singlehit[idx].max = hsinglediscri->GetBinCenter(lbmaxsingleADChit);
			fparam_singlehit[idx].min = hsinglediscri->GetBinCenter(lbminsingleADChit);
			
			fparam_singlehit[idx].isEmpty = false;
	...
	printf("SINGLE \t%.1lf\t%.1lf\t%.1lf\t%.1lf\n",fparam_singlehit[idx].mean,fparam_singlehit[idx].rms,fparam_singlehit[idx].min,fparam_singlehit[idx].max);	
	printf("-------------------------------------------------\n");
...
bool TAnalysis::ResetSpectrParam(){
	for (int i=0; i<4096; i++) {
		...
		fparam_singlehit[i].isEmpty = true;
	}
	return true;
...
int	TAnalysis::GetSingleHITEntries(int channel){
  int idx = channel;
	if(fparam_hit[idx].isEmpty==true)
	{	
		Extract();
	...
	return fparam_singlehit[idx].entries;
}
...
double	TAnalysis::GetSingleHITMean(int channel){
	int idx = channel;	
	if(fparam_singlehit[idx].isEmpty==true)
	{	
		Extract();
	}	
	return fparam_singlehit[idx].mean;
}
double	TAnalysis::GetSingleHITHigh(int channel){
	int idx = channel;	
	if(fparam_singlehit[idx].isEmpty==true)
	{	
		Extract();
	}	
	return fparam_singlehit[idx].max;
}
double	TAnalysis::GetSingleHITLow(int channel){
	int idx = channel;
	if(fparam_singlehit[idx].isEmpty==true)
	{	
		Extract();
	}	
	return fparam_singlehit[idx].min;
}
double	TAnalysis::GetSingleHITRms(int channel){
	int idx = channel;	
	if(fparam_singlehit[idx].isEmpty==true)
	{	
		Extract();
	}	
	return fparam_singlehit[idx].rms;
}
...
bool	TAnalysis::Extract(){
	...
	//TCut c_iter = Form("(Iteration$>=%d)&&(Iteration$<%d)",0,4095);
	TCut c_hit = Form("hit[Iteration$]==1");
	TH2F * ht2hit = new TH2F("ht2hit","Hit histo",4096,-.5,4095.5,4096, .5, 4096.5); //name,title,nbinx,xlow,xup,nbiny,ylow,yup
	ftree->Draw("ADC:Iteration$>>ht2hit",c_hit,"goff");	
		
	TCut c_hitsingle = Form("(hit[Iteration$]==1)&&(EvtMultiplicity==1)");
	TH2F * ht2hitsingle = new TH2F("ht2hitsingle","HIT histo single",4096,-.5,4095.5,4096, .5, 4096.5); //name,title,nbinx,xlow,xup,nbiny,ylow,yup
	ftree->Draw("ADC:Iteration$>>ht2hitsingle",c_hitsingle,"goff");	
...
//	cout <<" HIT binx " <<ht2hit->GetNbinsX()<<" ; biny =  "<< ht2hit->GetNbinsY()<<endl;
	...
	TH1F *hADC[4096]={NULL};
	TH1F *hHIT[4096]={NULL};
	TH1F *hSingleHIT[4096]={NULL};

	Int_t lbminADC = 0; 
	Int_t lbmaxADC = 0;
	Int_t lbminHIT = 0;
	Int_t lbmaxHIT = 0;
	Int_t lbminSingleHIT = 0;
	Int_t lbmaxSingleHIT = 0;
	Int_t k = 0;
	Int_t h = 0;
	Int_t q = 0;
	...
	for (int i=0; i<4096; i++) { // loop on channel 
		hADC[i] = (TH1F*) ht2adc->ProjectionY(Form("ht2adc_ch%d",i),i+1,i+1,"");	
		hHIT[i] = (TH1F*) ht2hit->ProjectionY(Form("ht2hit_ch%d",i),i+1,i+1,"");
		hSingleHIT[i] = (TH1F*) ht2hitsingle->ProjectionY(Form("ht2hitsingle_ch%d",i),i+1,i+1,"");
		...
		// search for limits of HIT histogram bin, content  !=0
		for(int z=0; z<4096; z++){ // loop on bins on hit histogram, 
			if(hSingleHIT[i]->GetBinContent(z)!=0){
				if(q==0){
					lbminSingleHIT=z;
				}
				q++;
		...
		fparam_singlehit[i].entries	= hSingleHIT[i]->GetEntries();
		fparam_singlehit[i].mean	= hSingleHIT[i]->GetMean();
		fparam_singlehit[i].rms		= hSingleHIT[i]->GetRMS();
		fparam_singlehit[i].max		= hSingleHIT[i]->GetBinCenter(lbmaxSingleHIT); 
		fparam_singlehit[i].min		= hSingleHIT[i]->GetBinCenter(lbminSingleHIT); 
		fparam_singlehit[i].isEmpty	= false;
...
Int_t TAnalysis::HitDisplay(int m,int opt, int first,int last,float sleep_val,const char * mapfile){
	...
	char nome[200];
	sprintf(nome,"./%s_COLZ.eps",fRunName);
	mycanv->DrawClone();
	mycanv->Print(nome);
	delete mycanv;
	CloseRootFile();
