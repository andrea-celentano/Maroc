...
#ifdef GICONF_DEBUG
				cout << __FUNCTION__ << " " << k << "/" << i << " loop passed (def/spe) " << (*ps).getPath() << " berr= 0x" << (hex) << berr << (dec) << endl;
#endif
			} // loop on i
			if ((k==0) && (ret==0)) { 
				break; 
...
public:
	...
	int parseFile(std::string filename) 
	{
		...
		if (fName != "") {cout << __FUNCTION__ << ": WARNING file " << filename << " will be parsed, overlapping previous " << fName << " config file" << endl;}

		try {cfg.readFile(filename.data());}
...
int parseInline(int argc, char *argv[]) {
   ...
   */
	...
	int insert(Tipo val, std::string path) {
  ...
  int navigate() {
	...
	//	Add an element to a list, it creates the list if new 
	//	
	// 	added by matteo 2013 Jan 23rd
	void AddList(const char * listname, int val)
	{
		Setting &root = cfg.getRoot();

		if(! root.exists(listname)){
				root.add(listname, Setting::TypeList);
		}
		
		Setting & stg = root[listname];
		
		stg.add(Setting::TypeInt) = val;		
		
	}
	
	// 	added by matteo 2013 Jan 23rd
	void AddListFloat(const char * listname, float val)
	{
		Setting &root = cfg.getRoot();
